...
class Board:
    ...
    def check_if_empty(self):
        ...
        if ((self.side1.bins.sum() == 0 or self.side2.bins.sum() == 0) or 
            ...
            for marbels in self.side1.bins:
                self.side1.store += marbels
                self.side1.playable_marbels = 0

            for marbels in self.side2.bins:
                self.side2.store += marbels
                self.side2.playable_marbels = 0

            # draw condition
            if self.side1.store == self.side2.store:
                if(self.side1.bins.sum() == 0): winner, loser = self.side2, self.side1
                else: winner, loser = self.side1, self.side2
            elif self.side1.store > self.side2.store: 
                winner, loser = self.side1, self.side2
            else: winner, loser = self.side2, self.side1

            # clear bins
            self.side1.bins = np.array([0] * NO_OF_BINS)
            self.side2.bins = np.array([0] * NO_OF_BINS)
    ...
    # consider weighted marbels -> keep as many as possible on left indexes
    def heuristic5(self, player, W1, W2, W3):
        close_stones1 = self.sides[player].weighted_marbels()
        close_stones2 = self.sides[1-player].weighted_marbels()
        return W1*self.heuristic1(player) + W2*(close_stones1 - close_stones2) + W3*self.sides[player].has_captured

    # considering close to winning
    def heuristic6(self, player, W1, W2, W3):
        ...
        return W1*(winning2 - winning1) + W2*(close_stones1 - close_stones2) + W3*self.sides[player].has_captured
