...
class Game:
    ...
    def set_heuristic(self, pc_hr, pc_hr_args, player_hr=None, player_hr_args=None, depth=[3,3], move_order=None):
    ...
    def get_next_move(self, player):
        ...
        max_val = minmax(n, player, self.additional_args['depth'][player], -np.inf, np.inf)
    ...
    def game_play_pc(self, verbose=True):
        ...
        while(True):
            ...
            # print("FINISH : ", finish)
            if finish: break
        if verbose: print("WINNER IS ", winner); self.board.print()
        return winner
...
if __name__ == "__main__":
    # np.random.seed(42)
    print("Enter 0 for human play, 1 for pc play")
    player ="human" if int(input()) == 0 else "pc"
    g = Game(player, first_move=True)
    move_order = list(range(NO_OF_BINS))
    np.random.shuffle(move_order)
    g.set_heuristic(1, [], 1, [], depth=[5, 5], move_order=move_order)
    g.game()